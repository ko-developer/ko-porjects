...
const CONNS = {
  ...
  nl8:     { n: 'ספיקון NL8 (רב-גידי)', c: '#bf360c' },
  ...
  strip2w: { n: 'כפול מוגן מים IP65', c: '#a32222', pw: 1, img: 'conn-strip2w.jpg', v: 1, cw: 76 },
  strip3w: { n: 'משולש מוגן מים IP65', c: '#a32222', pw: 1, img: 'conn-strip3w.jpg', v: 1, cw: 112 },
  strip4w: { n: 'רביעייה מוגנת מים IP65', c: '#a32222', pw: 1, img: 'conn-strip4w.jpg', v: 1, cw: 148 },
...
const BAND_LBL = { hi: 'HI', mid: 'MID', low: 'LOW' };
const BAND_PROXY = new Map();   /* 'nodeId#hi' → אובייקט שיורש מהמוקד החי (Object.create) עם id/band משלו */
const spkBase = id => String(id || '').split('#')[0];
const spkBand = id => { const i = String(id || '').indexOf('#'); return i >= 0 ? String(id).slice(i + 1) : ''; };
const byId = id => {
  if (typeof id === 'string' && id.includes('#')) {
    const base = P.nodes.find(n => n.id === spkBase(id)); if (!base) return undefined;
    let px = BAND_PROXY.get(id); if (!px || Object.getPrototypeOf(px) !== base) { px = Object.create(base); Object.defineProperty(px, 'id', { value: id, enumerable: true }); Object.defineProperty(px, 'band', { value: spkBand(id), enumerable: true }); BAND_PROXY.set(id, px); }
    return px;
  }
...
/* מוקד → יחידה בארון: שם/קטגוריה/גובה U מנוחשים מהשם, הפריט מההצעה (srcIid) והכבלים עוברים ליחידה */
function pointIntoRack(nid, rid, slot) {
  const n = byId(nid), r = byId(rid); if (!n || !r || r.kind !== 'rack') return;
  const name = (n.name || '').replace(/\s*\(\d+\)\s*$/, '').trim() || 'יחידה';
  const cat = /פאנל|panel|RP-|מולטי|patch|ניתוב/i.test(name) ? 'patch' : /פרוססור|processor|מטריצ|matrix|DSP|קרוסאובר/i.test(name) ? 'audio' : /מגבר|amplifier|\bamp\b/i.test(name) ? 'amp' : /רשת|switch|router|ראוטר|מתג/i.test(name) ? 'net' : 'other';
  const um = /(\d)\s?U\b/i.exec(name); const u = um ? Math.max(1, Math.min(6, +um[1])) : 1;
  let pos = slot >= 0 && fits(r, slot, u, -1) ? slot : firstFree(r, u);
  if (pos < 0) { uiToast('אין מקום פנוי בארון "' + r.name + '" — הגדל את גובה הארון'); render(); return; }
  const unit = { id: uid('u'), name: name.slice(0, 40), u, cat, pos, srcIid: n.srcIid };
  if (n.panel) unit.panel = n.panel;
  r.units.push(unit); rackArrange(r);
  let moved = 0;
  P.cables.forEach(c => { if (c.from === n.id) { c.from = r.id; c.fromUnit = unit.id; moved++; } if (c.to === n.id) { c.to = r.id; c.toUnit = unit.id; moved++; } });
  P.nodes = P.nodes.filter(x => x.id !== n.id);
  sel = r.id; ui.tab = 'node'; render(); save();
  uiToast('🗄 "' + name.slice(0, 30) + '" נכנס לארון "' + r.name + '" במיקום ' + (pos + 1) + 'U' + (moved ? ' · ' + moved + ' כבלים עברו איתו' : ''));
}
function addU(id, f) {
...
function shareDialog() {
  ...
  ov.querySelector('#shGo').onclick = async () => {
    ...
    try {
      ...
      const warn = j.localOnly ? `<div style="background:#fdeee8;border:1px solid #f3c9bd;border-radius:8px;padding:8px;margin-top:6px;font-size:12px;line-height:1.5;color:#8c2f16"><b>⚠ הקישור הזה נבנה על localhost ויעבוד רק במחשב שלך.</b> הגדר PUBLIC_URL ב-.env (כתובת השרת בענן) והפעל את השרת מחדש — הקישור ייבנה על הכתובת הציבורית.</div>` : '';
...
const REAR_KB = [
  ...
  { re: /\bID48\b/i, items: [{ t: 'power', label: 'AC' }, ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'xlrm', label: 'OUT' + k, port: 'OUT ' + k })), ...[4, 3, 2, 1].map(k => ({ t: 'xlrf', label: 'IN' + k, port: 'IN ' + k })), { t: 'rj45', label: 'NET' }] },
  { re: /DM88/i, items: [{ t: 'power', label: 'AC' }, ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'xlrm', label: 'OUT' + k, port: 'OUT ' + k })), ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'xlrf', label: 'IN' + k, port: 'IN ' + k })), ...['7-8', '5-6', '3-4', '1-2'].map(k => ({ t: 'xlrm', label: 'AES O' + k })), ...['7-8', '5-6', '3-4', '1-2'].map(k => ({ t: 'xlrf', label: 'AES I' + k })), { t: 'rj45', label: 'DANTE P' }, { t: 'rj45', label: 'DANTE S' }, { t: 'rj45', label: 'ETH 1' }, { t: 'rj45', label: 'ETH 2' }, { t: 'bnc', label: 'WCLK' }, { t: 'block4', label: 'GPI' }] },
  { re: /VMX88\s?L\b/i, items: [{ t: 'power', label: 'AC' }, ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'block3', label: 'O' + k, port: 'OUT ' + k })), ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'block3', label: 'I' + k, port: 'IN ' + k })), { t: 'rj45', label: 'ETH' }, { t: 'block4', label: 'GPI' }] },
  { re: /VMX88/i, items: [{ t: 'power', label: 'AC' }, ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'block3', label: 'O' + k, port: 'OUT ' + k })), ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'block3', label: 'I' + k, port: 'IN ' + k })), { t: 'rj45', label: 'DANTE P' }, { t: 'rj45', label: 'DANTE S' }, { t: 'rj45', label: 'ETH' }, { t: 'block4', label: 'GPI' }] },
  { re: /VMO16/i, items: [{ t: 'power', label: 'AC' }, ...Array.from({ length: 16 }, (_, i) => 16 - i).map(k => ({ t: 'block3', label: 'O' + k, port: 'OUT ' + k })), { t: 'rj45', label: 'DANTE P' }, { t: 'rj45', label: 'DANTE S' }, { t: 'rj45', label: 'ETH' }, { t: 'block4', label: 'GPI' }] },
  /* NST Audio */
  ...
  /* LAB GRUPPEN */
  { re: /IPD[\s-]?1200/i, items: [{ t: 'power', label: 'AC' }, { t: 'speakon', label: 'CH2', port: 'OUT 2' }, { t: 'speakon', label: 'CH1', port: 'OUT 1' }, { t: 'binding', label: 'BP1', port: 'OUT 1' }, { t: 'binding', label: 'BP2', port: 'OUT 2' }, { t: 'xlrf', label: 'IN1', port: 'IN 1' }, { t: 'xlrm', label: 'LNK1', port: 'LNK 1' }, { t: 'xlrf', label: 'IN2', port: 'IN 2' }, { t: 'xlrm', label: 'LNK2', port: 'LNK 2' }, { t: 'xlrf', label: 'AES', port: 'IN 3' }, { t: 'xlrm', label: 'AES LNK', port: 'LNK 3' }, { t: 'rj45', label: 'ETH' }] },   /* לפי צילום הגב של סדרת IPD: 2 כניסות אנלוגיות + לינק, AES3 + לינק, Ethernet, ספיקון לכל ערוץ, IEC */
  { re: /IPD[\s-]?2400/i, items: [{ t: 'power', label: 'AC' }, { t: 'speakon', label: 'CH2', port: 'OUT 2' }, { t: 'speakon', label: 'CH1', port: 'OUT 1' }, { t: 'binding', label: 'BP1', port: 'OUT 1' }, { t: 'binding', label: 'BP2', port: 'OUT 2' }, { t: 'xlrf', label: 'IN1', port: 'IN 1' }, { t: 'xlrm', label: 'LNK1', port: 'LNK 1' }, { t: 'xlrf', label: 'IN2', port: 'IN 2' }, { t: 'xlrm', label: 'LNK2', port: 'LNK 2' }, { t: 'xlrf', label: 'AES', port: 'IN 3' }, { t: 'xlrm', label: 'AES LNK', port: 'LNK 3' }, { t: 'rj45', label: 'ETH' }] },   /* לפי צילום הגב של סדרת IPD: 2 כניסות אנלוגיות + לינק, AES3 + לינק, Ethernet, ספיקון לכל ערוץ, IEC */
  { re: /PLM\s?12K44|PLM\s?20K44/i, items: [{ t: 'power', label: 'AC' }, { t: 'speakon', label: 'ALL 1-4' }, ...[1, 2, 3, 4].map(k => ({ t: 'speakon', label: 'CH' + k, port: 'OUT ' + k })), ...[1, 2, 3, 4].map(k => ({ t: 'xlrf', label: 'IN' + k, port: 'IN ' + k })), { t: 'xlrf', label: 'AES 1-2', port: 'IN 5' }, { t: 'xlrf', label: 'AES 3-4', port: 'IN 6' }, { t: 'rj45', label: 'DANTE P' }, { t: 'rj45', label: 'DANTE S' }] },   /* לפי צילום הגב הרשמי: NLT8 לכל 4 הערוצים + 2×NLT4 (1-2, 3-4), 4 כניסות אנלוגיות Iso-Float, 2 AES/EBU, Dante כפול, powerCON */
  /* MAGNETIC */
  /* פרוססורים */
  { re: /DC\s?1048|XTA.*DC10/i, items: [{ t: 'power', label: 'AC' }, { t: 'block3', label: 'RS485' }, { t: 'block8', label: 'GPIO' }, ...[8, 7, 6, 5, 4, 3, 2, 1].map(k => ({ t: 'block3', label: 'OUT' + k, port: 'OUT ' + k })), ...[4, 3, 2, 1].map(k => ({ t: 'block3', label: 'IN' + k, port: 'IN ' + k }))] },   /* לפי צילום הגב: הכל בלוקי Phoenix, כניסות D–A */
  ...
  { re: /DSK\s?3\.?1|DIGI.*DSK/i, items: [{ t: 'power', label: 'AC' }, { t: 'db9', label: 'RS232' }, { t: 'jack', label: 'MIC C', port: 'IN 7' }, { t: 'rca', label: 'AUX L' }, { t: 'rca', label: 'AUX R' }, { t: 'rca', label: 'V 1' }, { t: 'rca', label: 'V 2' }, { t: 'rca', label: 'V 3' }, { t: 'rca', label: 'V 4' }, { t: 'xlrm', label: 'SUB', port: 'OUT 1' }, { t: 'xlrm', label: 'OUT R', port: 'OUT 2' }, { t: 'jack', label: 'MIC 1', port: 'IN 3' }, { t: 'jack', label: 'MIC 2', port: 'IN 4' }, { t: 'xlrf', label: 'BGM', port: 'IN 1' }, { t: 'xlrm', label: 'OUT L', port: 'OUT 3' }, { t: 'jack', label: 'MIC 3', port: 'IN 5' }, { t: 'jack', label: 'MIC 4', port: 'IN 6' }, { t: 'xlrf', label: 'MIC IN', port: 'IN 2' }] },   /* לפי צילום הגב שהתקבל — הכיתוב בצילום קטן, לאמת בשטח */
  /* מיקסרים לראק */
  { re: /XTA\s*(DNA|APA)|DNA\s?20/i, items: ampRear(4, 4, { net: 'ETH', link: true }) },
  /* K&F IPX / IX = Dynacord IPX / IX (אותו מוצר) — פריסות לפי הצילומים מ-products.dynacord.com: Euroblock ליציאות ולכניסות, OMNEO/Dante כפול, GPIO */
  { re: /IPX\s?10\s?:?\s?8/i, items: [{ t: 'power', label: 'AC' }, ...[1, 2, 3, 4, 5, 6, 7, 8].map(k => ({ t: 'block2', label: 'CH' + k, port: 'OUT ' + k })), ...[1, 2, 3, 4, 5, 6, 7, 8].map(k => ({ t: 'block3', label: 'IN' + k, port: 'IN ' + k })), { t: 'rj45', label: 'DANTE P' }, { t: 'rj45', label: 'DANTE S' }, { t: 'block8', label: 'GPIO' }] },
  { re: /IPX\s?(5|10|20)\s?:?\s?4|IPX\s?5\b|IPX10\b|IPX\s?20\b/i, items: [{ t: 'power', label: 'AC' }, ...[1, 2, 3, 4].map(k => ({ t: 'block2', label: 'CH' + k, port: 'OUT ' + k })), ...[1, 2, 3, 4].map(k => ({ t: 'block3', label: 'IN' + k, port: 'IN ' + k })), { t: 'rj45', label: 'DANTE P' }, { t: 'rj45', label: 'DANTE S' }, { t: 'block8', label: 'GPIO' }] },
  { re: /\bIX\s?15\s?:?\s?4|\bIX\s?15\b/i, items: [{ t: 'power', label: 'AC' }, ...[1, 2, 3, 4].map(k => ({ t: 'block2', label: 'CH' + k, port: 'OUT ' + k })), ...[1, 2, 3, 4].map(k => ({ t: 'block3', label: 'IN' + k, port: 'IN ' + k })), { t: 'rj45', label: 'DANTE P' }, { t: 'rj45', label: 'DANTE S' }, { t: 'block8', label: 'GPIO' }] },
...
function rearPretty(re) {
  let t = String(re instanceof RegExp ? re.source : re).replace(/^\/|\/i?$/g, '');
  t = t.replace(/\((?:[A-Z0-9]+\|)+[A-Z0-9]+\)/gi, m => m.slice(1, -1).replace(/\|/g, '/'));
  t = t.split('|')[0].replace(/\\\.\??/g, '.').replace(/\.\*/g, ' ').replace(/\[[^\]]*\]\??/g, ' ')
    .replace(/\\d\??|\\s\?|\\s\*|\\s|\\b|\(|\)|\?|\^|\$/g, ' ').replace(/\s*:\s*/g, ':').replace(/\s+/g, ' ').trim();
  return t;
}
/* פריסות הגב של השרת — data/rear_layouts.json (מה שהעורך על התמונה שומר; תקף לכל הפרויקטים).
   התאמה: קודם לפי שם הרשומה, אחר כך לפי ה-regex — הרשומות הספציפיות קודמות לכלליות */
function rearSrvEntry(name) {
  if (typeof REAR_LAYOUTS === 'undefined' || !name) return null;
  const key = rearKey(name);
  return REAR_LAYOUTS.find(r => r.name === key) || REAR_LAYOUTS.find(r => { try { return new RegExp(r.re, 'i').test(name); } catch { return false; } }) || null;
}
...
function rearLayoutSrc(name) {
  const lib = (store && store.rearLib) || {};
  const lk = rearLibKey(name), sv = rearSrvEntry(name);
  /* ספריית השרת (נערך על התמונה) גוברת על עותק ישן בפרויקט; עותק בפרויקט גובר רק אם נשמר אחרי גרסת השרת (עריכה בלי שרת, למשל ב-Netlify) */
  const libT = lk ? ((store && store.rearLibMeta) || {})[lk] || '' : '';
  if (lk && (!sv || libT > (sv.updated || ''))) return { items: lib[lk], src: 'lib', key: lk };
  if (sv) return { items: sv.items, src: 'srv', key: sv.name, re: sv.re };
...
function rearVerified(name) { const s = rearLayoutSrc(name).src; return s === 'lib' || s === 'srv'; }
...
function rearGlyph(t, label) {
  if (t === 'rca') { /* RCA בצבע המחבר האמיתי: L = לבן, R = אדום, קואקס = כתום, אחר = זהב */
    ...
    const c = /^V\b|VIDEO|CVBS/.test(lb) ? '#f2c94c' : /(^|\s)L$/.test(lb) ? '#f2f2f2' : /(^|\s)R$/.test(lb) ? '#d62828' : /COAX/.test(lb) ? '#f08c00' : '#c98a2e';
  ...
  if (t === 'db9') { /* D-Sub 9 (RS232/RS485): טרפז מתכתי עם 2 שורות פינים (5+4) */
    let pins = ''; for (let k = 0; k < 5; k++) pins += `<circle cx="${(5.2 + k * 2.9).toFixed(1)}" cy="9.2" r="0.8" fill="#222"/>`; for (let k = 0; k < 4; k++) pins += `<circle cx="${(6.65 + k * 2.9).toFixed(1)}" cy="12.8" r="0.8" fill="#222"/>`;
    return `<svg ${S}><rect x="1" y="6" width="20" height="10" rx="2" fill="#3a3f4a"/><path d="M3.5 7.5h15l-1.6 7h-11.8z" fill="#cfd3da" stroke="#777" stroke-width=".6"/>${pins}<circle cx="2.4" cy="11" r="0.9" fill="#999"/><circle cx="19.6" cy="11" r="0.9" fill="#999"/></svg>`;
  }
...
const REAR_TYPES = [['speakon', 'ספיקון NL4'], ['xlrf', 'XLR נקבה'], ['xlrm', 'XLR זכר'], ['multi', 'מולטי XLR'], ['rj45', 'רשת RJ45 / Cat6'], ['fiber', 'אופטי LC/SC'], ['dmx', 'DMX (XLR 5/3)'], ['bnc', 'BNC / SDI'], ['hdmi', 'HDMI / וידאו'], ['rca', 'RCA'], ['binding', 'בורג/בננה'], ['block2', 'בלוק Phoenix 2 פינים (קו רמקול)'], ['block3', 'בלוק Phoenix 3 פינים (אודיו מאוזן)'], ['block4', 'בלוק Phoenix 4 פינים'], ['block8', 'בלוק רב-פינים (GPIO / בקרה)'], ['jack', 'ג׳ק 6.35 (מיקרופון/קו)'], ['usb', 'USB'], ['db9', 'D-Sub 9 (RS232 / RS485)'], ['power', 'חשמל'], ['dip', 'מתגי DIP']];
...
function rearEditorByName(uname) {
  ...
  /* הרשומה בשרת (data/rear_layouts.json) שאליה נשמור: הרשומה הקיימת → בסיס הידע המובנה → השם עצמו */
  const sv = rearSrvEntry(uname), kb = REAR_KB.find(e => e.re.test(uname || ''));
  window.__rearEntry = sv ? { name: sv.name, re: sv.re } : kb ? { name: rearPretty(kb.re), re: kb.re.source } : { name: window.__rearName, re: window.__rearName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') };
  const srcTxt = { lib: 'מותאם בפרויקט הזה (יעבור לשרת בשמירה)', srv: 'ספריית השרת — נערך על התמונה ✓', kb: 'בסיס ידע מובנה — עדיין לא אושר על התמונה', io: 'ניחוש לפי סוג המוצר', generic: 'ניחוש כללי' }[src.src] || src.src;
  const ov = document.createElement('div');
  ov.id = 'rearEdOv';
  ...
  ov.innerHTML = `<div style="background:#fff;border-radius:12px;padding:16px;max-width:760px;width:96%;max-height:90vh;overflow-y:auto;box-shadow:0 12px 40px rgba(0,0,0,.35)">
    ...
    <p class="muted" style="margin:0 0 8px">מקור הפריסה: <b>${srcTxt}</b> · השמירה עוברת לשרת (data/rear_layouts.json) ותקפה לכל הפרויקטים והדוחות.</p>
    <div class="fld" style="margin-bottom:8px"><label>📋 משוך פריסת גב ממוצר אחר (מעתיק את כל המחברים)</label>
      <select onchange="if(this.value!==''){rearCopyFrom(this.value);this.value='';}">
        ...
        ${(() => {
          ...
          if (typeof REAR_LAYOUTS !== 'undefined') REAR_LAYOUTS.forEach(r => names.add(r.name));
          REAR_KB.forEach(e => names.add(rearPretty(e.re)));
    ...
    <div style="display:flex;gap:8px;margin-top:12px">
      <button class="primary" style="flex:1" onclick="rearSave()">💾 שמור לספרייה (שרת)</button>
      <button style="flex:1" onclick="document.getElementById('rearEdOv').remove()">ביטול</button>
    </div></div>`;
  document.body.appendChild(ov);
  document.removeEventListener('keydown', rearEdKeys); document.addEventListener('keydown', rearEdKeys);
  rearEdRender();
}
...
function rearEdRender() {
  ...
  const im = rearImage(window.__rearName), hasImg = !!(im && (im.url || im.furl));
  const RZ = window.__rearZoom || 100, NM = window.__rearNum;
  let h = '';
  if (hasImg) {
    /* העורך על התמונה: המחברים במקומם האמיתי — גרירה, לחיצה כפולה להוספה, מספור מהיר */
    h += `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;font-size:11px"><span class="muted">זום:</span><button class="mini" onclick="rearZoom(-50)">−</button><b style="min-width:42px;text-align:center">${RZ}%</b><button class="mini" onclick="rearZoom(50)">+</button><input type="range" min="100" max="500" step="25" value="${RZ}" oninput="rearZoom(+this.value, true)" style="flex:1"><button class="mini" onclick="rearZoom(100, true)">התאם</button></div>
    <div class="rearscroll" style="overflow:auto;max-height:60vh;border-radius:8px"><div style="width:${RZ}%">${rearImageHTML(window.__rearName, items, { edit: true, sel })}</div></div>
    ...
    <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;background:${NM ? '#fff1e3' : '#f4f2ec'};border:1px solid ${NM ? '#ff8a50' : 'transparent'};border-radius:8px;padding:5px 8px;font-size:11px;margin-bottom:6px">
      <b>🔢 מספור מהיר:</b>
      <select id="rearNumRole" style="width:auto;font-size:11px"><option value="OUT" ${NM && NM.role === 'OUT' ? 'selected' : ''}>יציאות OUT</option><option value="IN" ${NM && NM.role === 'IN' ? 'selected' : ''}>כניסות IN</option><option value="LNK" ${NM && NM.role === 'LNK' ? 'selected' : ''}>לינק LNK</option></select>
      <span>מ-</span><input id="rearNumStart" type="number" min="1" max="64" value="${NM ? NM.n : 1}" style="width:54px;font-size:11px">
      ${NM ? `<b style="color:#c94a24">הבא: ${NM.role} ${NM.n}</b><button class="mini" onclick="rearNumMode(false)">עצור (Esc)</button>` : `<button class="mini" onclick="rearNumMode(true)">התחל</button><span class="muted">ואז לחץ על המחברים לפי הסדר — כל לחיצה משייכת את המספר הבא</span>`}
    </div>`;
  } else {
    /* אין תמונה — פאנל סכמטי לפי הסדר (יציאות משמאל, כניסות מימין) */
    ...
    h += `<div style="background:#16191f;border-radius:8px;padding:12px 10px;display:flex;gap:2px;align-items:center;overflow-x:auto;direction:ltr">
  ...
  panel.innerHTML = h;
  if (hasImg) rearBindDrag(panel);
  if (sel < 0 || sel >= items.length) { edit.innerHTML = ''; return; }
  const it = items[sel];
  const pm = /^(OUT|IN|LNK)\s*(\d+)/i.exec(it.port || '');
  const pos = (im && im.pos) || {};
  const cur = hasImg ? rearPosOf(it, pos, sel, items.length) : null;
  const curW = cur && cur.w != null ? cur.w : (/^block/.test(it.t) ? 4.2 : 2.6);
  edit.innerHTML = `<div style="background:#f4f2ec;border-radius:8px;padding:10px;margin-top:6px">
    <div style="font-size:11px;margin-bottom:6px"><b>מחבר ${sel + 1} מתוך ${items.length}</b> — ${esc(it.label || '(ללא תווית)')}${it.port ? ' · <b style="color:' + (/^IN/i.test(it.port) ? '#0f6e56' : '#c94a24') + '">' + esc(it.port) + '</b>' : ''}</div>
    <div class="row2">
      ...
      <div class="fld"><label>תווית (הכיתוב על המוצר)</label><input value="${esc(it.label || '')}" placeholder="למשל CH1 / IN1 / AUX2" onchange="__rearDraft[${sel}].label=this.value;rearEdRender()"></div>
    </div>
    <div class="row2">
      <div class="fld"><label>משויך לפורט (מספר היציאה/הכניסה בחיווט)</label><div style="display:flex;gap:4px">
        <select style="flex:1" onchange="rearSetPort(${sel},this.value,null)"><option value="" ${!pm ? 'selected' : ''}>— ללא (חשמל / רשת / בקרה) —</option><option value="OUT" ${pm && pm[1].toUpperCase() === 'OUT' ? 'selected' : ''}>יציאה OUT</option><option value="IN" ${pm && pm[1].toUpperCase() === 'IN' ? 'selected' : ''}>כניסה IN</option><option value="LNK" ${pm && pm[1].toUpperCase() === 'LNK' ? 'selected' : ''}>לינק LNK (יציאת שרשור)</option></select>
        <input type="number" min="1" max="64" value="${pm ? +pm[2] : ''}" placeholder="מס׳" style="width:64px" ${pm ? '' : 'disabled'} onchange="rearSetPort(${sel},null,this.value)"></div></div>
      ${(im || {}).furl ? `<div class="fld"><label>איפה המחבר יושב</label><select onchange="rearSetSide(${sel},this.value)"><option value="rear" ${rearSideOf(it, pos) !== 'front' ? 'selected' : ''}>גב</option><option value="front" ${rearSideOf(it, pos) === 'front' ? 'selected' : ''}>חזית</option></select></div>` : '<div></div>'}
    </div>
    ${hasImg ? `<div class="row2">
      <div class="fld"><label>רוחב הסמן על התמונה (% מרוחב התמונה) — שיכסה בדיוק את המחבר</label><div style="display:flex;gap:4px;align-items:center"><input type="range" min="0.8" max="16" step="0.1" value="${curW}" style="flex:1" oninput="rearSetW(${sel},+this.value,true)" onchange="rearSetW(${sel},+this.value)"><b id="rearWv" style="min-width:40px;font-size:11px">${it.w != null ? it.w + '%' : 'אוטו'}</b><button class="mini" onclick="rearSetW(${sel},null)">אוטו</button></div></div>
      <div class="fld"><label>מיקום מדויק (% מהתמונה — X / Y)</label><div style="display:flex;gap:4px"><input type="number" step="0.1" min="0" max="100" value="${cur.x.toFixed(1)}" onchange="rearSetXY(${sel},+this.value,null)"><input type="number" step="0.1" min="0" max="100" value="${cur.y.toFixed(1)}" onchange="rearSetXY(${sel},null,+this.value)"></div></div>
    </div>` : ''}
    <div style="display:flex;gap:6px;margin-top:4px">
      ${hasImg ? '' : `<button style="flex:1" onclick="rearMove(${sel},-1)" ${sel === 0 ? 'disabled' : ''}>◀ הזז שמאלה</button>
      <button style="flex:1" onclick="rearMove(${sel},1)" ${sel === items.length - 1 ? 'disabled' : ''}>הזז ימינה ▶</button>`}
      <button style="flex:1;background:#f3d9d2;color:#8c2f16" onclick="__rearDraft.splice(${sel},1);__rearSel=Math.max(0,${sel}-1);rearEdRender()">✕ מחק מחבר</button>
    </div>
    ${hasImg ? '' : `<div style="display:flex;gap:6px;margin-top:6px">
...
/* גרירת סמני המחברים על תמונת הגב — מיקום באחוזים, נשמר בפריט (x,y). לחיצה במצב מספור = שיוך המספר הבא. לחיצה כפולה על מקום ריק = מחבר חדש שם */
...
function rearBindDragBox(box) {
  let drag = null;
  const pct = e => { const r = box.getBoundingClientRect(); return { x: Math.max(0, Math.min(100, (e.clientX - r.left) / r.width * 100)), y: Math.max(0, Math.min(100, (e.clientY - r.top) / r.height * 100)) }; };
  box.addEventListener('pointerdown', e => {
    const mk = e.target.closest('.rmk'); if (!mk) return;
    const i = +mk.dataset.ri; window.__rearSel = i; drag = { i, moved: false, sx: e.clientX, sy: e.clientY };
    mk.setPointerCapture(e.pointerId); e.preventDefault();
  });
  box.addEventListener('pointermove', e => {
    if (!drag) return;
    if (!drag.moved && Math.hypot(e.clientX - drag.sx, e.clientY - drag.sy) < 3) return;   /* לחיצה קצרה ≠ גרירה */
    const { x, y } = pct(e);
...
/* מצב מספור מהיר: OUT/IN/LNK החל ממספר — כל לחיצה על מחבר בתמונה משייכת את המספר הבא */
function rearNumMode(on) {
  if (!on) { window.__rearNum = null; rearEdRender(); return; }
  const role = (document.getElementById('rearNumRole') || {}).value || 'OUT', n = +((document.getElementById('rearNumStart') || {}).value) || 1;
  window.__rearNum = { role, n }; rearEdRender();
}
function rearNumAssign(i) {
  const it = window.__rearDraft[i], NM = window.__rearNum; if (!it || !NM) return;
  it.port = NM.role + ' ' + NM.n;
  if (!it.label) it.label = NM.role + NM.n;
  NM.n++;
}
function rearSetPort(i, role, n) {
  const it = window.__rearDraft[i]; if (!it) return;
  const m = /^(OUT|IN|LNK)\s*(\d+)/i.exec(it.port || '');
  const r = role != null ? role : (m ? m[1].toUpperCase() : 'OUT');
  const k = n != null ? Math.max(1, Math.min(64, +n || 1)) : (m ? +m[2] : 1);
  if (!r) delete it.port; else it.port = r + ' ' + k;
  rearEdRender();
}
function rearSetW(i, w, live) {
  const it = window.__rearDraft[i]; if (!it) return;
  if (w == null) delete it.w; else it.w = Math.round(w * 10) / 10;
  if (live) {
    document.querySelectorAll(`#rearEdPanel .rmk[data-ri="${i}"]`).forEach(mk => { mk.style.width = it.w ? it.w + '%' : ''; mk.style.maxWidth = it.w ? 'none' : ''; mk.style.minWidth = it.w ? '0' : ''; });
    const v = document.getElementById('rearWv'); if (v) v.textContent = it.w ? it.w + '%' : 'אוטו';
    return;
  }
  rearEdRender();
}
function rearSetXY(i, x, y) {
  const it = window.__rearDraft[i]; if (!it) return;
  const im = rearImage(window.__rearName), cur = rearPosOf(it, (im && im.pos) || {}, i, window.__rearDraft.length);
  it.x = Math.round(Math.max(0, Math.min(100, x != null ? x : cur.x)) * 10) / 10;
  it.y = Math.round(Math.max(0, Math.min(100, y != null ? y : cur.y)) * 10) / 10;
  rearEdRender();
}
function rearDup() {
  const a = window.__rearDraft, i = window.__rearSel, src = a[i]; if (!src) return;
  const c = JSON.parse(JSON.stringify(src));
  const im = rearImage(window.__rearName), cur = rearPosOf(src, (im && im.pos) || {}, i, a.length);
  c.x = Math.min(100, Math.round((cur.x + (cur.w || 2.6) + 0.4) * 10) / 10); c.y = Math.round(cur.y * 10) / 10;
  const m = (c.label || '').match(/^(.*?)(\d+)\s*$/); if (m) c.label = m[1] + (+m[2] + 1);
  const pm = (c.port || '').match(/^(OUT|IN|LNK)\s*(\d+)/i); if (pm) c.port = pm[1].toUpperCase() + ' ' + (+pm[2] + 1);
  a.splice(i + 1, 0, c); window.__rearSel = i + 1; rearEdRender();
}
/* מקלדת בעורך: חצים = הזזה עדינה של הסמן הנבחר (Shift = 1%), Delete = מחיקה, Esc = יציאה ממצב מספור */
function rearEdKeys(e) {
  ...
  else if (e.key === 'Delete' || e.key === 'Backspace') { a.splice(i, 1); window.__rearSel = Math.min(i, a.length - 1); rearEdRender(); e.preventDefault(); }
}
...
/* מחבר חדש: כמו הנבחר/האחרון עם תווית ופורט מוגדלים ב-1. עם תמונה — במקום הלחיצה הכפולה (at) או במרכז */
function rearAddItem(at) {
  const a = window.__rearDraft, last = a[window.__rearSel] || a[a.length - 1];
  ...
  const it = { t, label, port };
  const im = rearImage(window.__rearName);
  if (at) { it.x = Math.round(at.x * 10) / 10; it.y = Math.round(at.y * 10) / 10; if (at.side === 'front') it.side = 'front'; }
  else if (im) { it.x = 50; it.y = 50; }
  if (im && last && last.w) it.w = last.w;
  a.push(it);
...
/* שמירה לשרת (data/rear_layouts.json) — תקף לכל הפרויקטים. בלי שרת (Netlify) — נשמר בפרויקט (store.rearLib) כמו קודם */
async function rearSrvSave(rec) {
  try {
    const r = await fetch('/api/rear-layout', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(rec) });
    if (!r.ok) return null;
    const j = await r.json(); if (!j.ok || !j.rec) return null;
    if (typeof REAR_LAYOUTS !== 'undefined') { const k = REAR_LAYOUTS.findIndex(x => x.name === j.rec.name || x.re === j.rec.re); if (k >= 0) REAR_LAYOUTS[k] = j.rec; else REAR_LAYOUTS.unshift(j.rec); }
    return j.rec;
  } catch { return null; }
}
async function rearSave() {
  const key = window.__rearName, items = window.__rearDraft.filter(it => it.t).map(it => ({ ...it }));
  /* מקבעים לכל מחבר את מיקומו בפועל (גם מי שעדיין יושב על המיקום המוכן מהתמונה) — הרשומה בשרת שלמה ועצמאית */
  const im = rearImage(key);
  if (im) items.forEach((it, i) => {
    const p = rearPosOf(it, im.pos || {}, i, items.length);
    if (it.x == null) { it.x = Math.round(p.x * 10) / 10; it.y = Math.round(p.y * 10) / 10; }
    if (it.w == null && p.w != null) it.w = p.w;
    if (!it.side && rearSideOf(it, im.pos || {}) === 'front') it.side = 'front';
    if (it.port === undefined) delete it.port;
  });
  const ent = window.__rearEntry || { name: key, re: key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') };
  const rec = await rearSrvSave({ name: ent.name, re: ent.re, img: im ? im.re : undefined, items });
  if (rec) {
    const lk = rearLibKey(key); if (lk && store.rearLib) { delete store.rearLib[lk]; if (store.rearLibMeta) delete store.rearLibMeta[lk]; }   /* העותק בפרויקט נבלע בשרת — אין יותר כפילות */
    uiToast('✓ גב ' + rec.name + ' נשמר בשרת — תקף לכל הפרויקטים');
  } else {
    store.rearLib = store.rearLib || {};
    store.rearLib[key] = items;
    (store.rearLibMeta = store.rearLibMeta || {})[key] = new Date().toISOString();   /* חדש מגרסת השרת → גובר עליה */
    uiToast('נשמר בפרויקט (אין שרת זמין — בגרסה עם שרת יישמר לספרייה המשותפת)');
  }
  save();
  const ov = document.getElementById('rearEdOv'); if (ov) ov.remove();
  render();
}
/* העברת פריסה מותאמת מהפרויקט אל השרת (מהספרייה) */
async function rearLibPush(name) {
  const items = (store.rearLib || {})[name]; if (!items) return;
  const sv = rearSrvEntry(name), kb = REAR_KB.find(e => e.re.test(name));
  const ent = sv ? { name: sv.name, re: sv.re } : kb ? { name: rearPretty(kb.re), re: kb.re.source } : { name, re: name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') };
  const im = rearImage(name);
  const rec = await rearSrvSave({ name: ent.name, re: ent.re, img: im ? im.re : undefined, items });
  if (!rec) { alert('אין שרת זמין — אפשר להעביר רק מהמחשב שמריץ את השרת'); return; }
  delete store.rearLib[name]; if (store.rearLibMeta) delete store.rearLibMeta[name]; save(); uiToast('✓ ' + rec.name + ' הועבר לשרת');
  const ov = document.getElementById('rearLibOv'); if (ov) { ov.remove(); rearLibManager(); }
}
async function rearSrvDelete(name) {
  if (!(await uiConfirm('למחוק את פריסת "' + name + '" מספריית השרת? (יחזור לבסיס הידע המובנה אם קיים)'))) return;
  try {
    const r = await fetch('/api/rear-layout', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name, remove: true }) });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    if (typeof REAR_LAYOUTS !== 'undefined') { const k = REAR_LAYOUTS.findIndex(x => x.name === name); if (k >= 0) REAR_LAYOUTS.splice(k, 1); }
    const ov = document.getElementById('rearLibOv'); if (ov) { ov.remove(); rearLibManager(); }
    render();
...
function rearLibManager() {
  ...
  for (const [name, items] of Object.entries(lib)) {
    const shadow = rearLayoutSrc(name).src === 'srv';   /* יש גרסת שרת חדשה יותר — העותק בפרויקט לא בשימוש */
    rows.push({ name, items, n: items.length, src: shadow ? 'עותק ישן בפרויקט — גרסת השרת גוברת' : 'מותאם בפרויקט', custom: true });
  }
  /* ספריית השרת (data/rear_layouts.json) — הפריסות שנערכו/הוגרו על התמונות; גוברות על בסיס הידע המובנה */
  (typeof REAR_LAYOUTS !== 'undefined' ? REAR_LAYOUTS : []).forEach(r => {
    if (!rows.some(x => x.name === r.name)) rows.push({ name: r.name, items: r.items, n: r.items.length, src: r.src === 'editor' ? 'שרת · נערך ✓' : 'שרת · הוגר', srv: true, custom: false, re: r.re });
  });
  REAR_KB.forEach(e => {
    const nm = rearPretty(e.re);
    if (!rows.some(r => r.name === nm || r.re === e.re.source)) rows.push({ name: nm, items: e.items, n: e.items.length, src: 'מובנה', custom: false, re: e.re.source });
  });
  /* מותג, סוג מוצר (לפי הרכב המחברים) ומספר יציאות — לקיבוץ ולמיון */
  const REAR_BRANDS = [['XTA', /XTA|DPA|DNA|\bAPA\b|DS8000|MX36|\bSIX\b|DC\s?1048/i], ['Kling & Freitag', /K&F|KLING|IPX|\bIX\s?\d|SCALA|TGX|TOPAS|\bD\s?\d{2,3}\s?:\s?4\b/i], ['Funktion-One', /FUNKTION|\bD\d{2,3}Q/i], ['NST Audio', /NST|D48|ID48|D24S|VMX88|VMO16|DM88/i], ['SAE', /SAE|PQM|\bMA\s?\d|MAX\s?\d/i], ['DigiSynthetic', /DIGISYNTHET|DIGISYNTHEIC|\bDS\s?\d{3}[A-Z]?\b|DMX\s?\d{3,4}\s?[A-Z]?|DSK\s?\d|\bK[57]\b/i], ['KT Audio', /\bKT\b|UNICORN|DYNAMIQ|MX3|XLI\s?2500/i], ['Lab.gruppen', /LAB|PLM|IPD/i], ['Powersoft', /POWERSOFT|QUATTRO|OTTOCANALI/i], ['Symetrix', /SYMETRIX|PRISM|JUPITER/i], ['Midas', /MIDAS/i], ['Behringer', /BE[RH]?RINGER/i]];
  ...
  const TYPE_ORDER = ['פרוססורים / אודיו', 'מגברים', 'מגברי DSP', 'מגברי DSP עם יציאות AUX', 'רשת / אופטי', 'וידאו', 'תאורה', 'אחר'];   /* בתוך מותג: פרוססורים למעלה, מגברים למטה */
  ...
  ov.innerHTML = `<div style="background:#fff;border-radius:12px;padding:16px;max-width:560px;width:94%;max-height:86vh;overflow-y:auto;box-shadow:0 12px 40px rgba(0,0,0,.35)">
    ...
    <p class="muted" style="margin:0 0 8px;font-size:11px">✎ = עריכה על התמונה (מיקום, סוג, מספר הפורט) — נשמר בשרת ותקף לכל הפרויקטים · "שרת · נערך ✓" = אושר על התמונה · "הוגר" = הועבר אוטומטית מבסיס הידע וממתין לאישור · "מובנה" = גיבוי בקוד בלבד · 📷 ${rows.filter(r => rearImage(r.name)).length} דגמים עם תמונה.</p>
...
function connGlyph(conn) {
  ...
  switch (conn) {
    ...
    case 'nl8': return `<svg ${S}><circle cx="11" cy="11" r="9" fill="#fff" stroke="${C}" stroke-width="1.8"/><rect x="9.6" y="2" width="2.8" height="4.2" fill="${C}"/><circle cx="11" cy="12" r="4.2" fill="none" stroke="${C}" stroke-width="1.4"/><text x="11" y="14" text-anchor="middle" font-size="5.5" font-weight="800" fill="${C}">8</text></svg>`;
...
function pairCabType(n, i) {
  ...
  return /rj45/.test(k) ? 'cat' : /hdmi/.test(k) ? 'sdi:aoc' : /bnc/.test(k) ? 'sdi' : /fiber/.test(k) ? 'fiber' : /speakon/.test(k) ? 'nl4' : /dmx/.test(k) ? 'dmx' : /pwr|^si16|^cee|^strip/.test(k) ? 'pwr' : 'xlr';
}
...
function pairCableOf(nid, i) {
  ...
  return P.cables.find(c => ((c.from === nid && c.to === n.pair && c.fromHole === i + 1) || (c.from === n.pair && c.to === nid && c.toHole === i + 1))
    || (c.chans && ((c.from === nid && c.to === n.pair) || (c.from === n.pair && c.to === nid)) && c.chans.some(x => x.a === i + 1))) || null;
}
/* משפחות שאפשר לאחד לכבל מולטי אחד עם N ליבות: XLR → מולטי XLR, RJ45 → מולטי רשת, אופטי → מולטי סיבים */
const PAIR_FAM = { xlr: { test: k => /^xlr/.test(k), type: 'multi', n: 'מולטי XLR', conn: 'xlrm', conn2: 'xlrf' }, cat: { test: k => k === 'rj45', type: 'cat', n: 'מולטי רשת (Cat6)', conn: 'rj45', conn2: 'rj45' }, fiber: { test: k => k === 'fiber', type: 'fiber', n: 'מולטי אופטי', conn: 'fiber', conn2: 'fiber' } };
function pairFamOf(k) { for (const [f, d] of Object.entries(PAIR_FAM)) if (d.test(k || '')) return f; return null; }
function pairMultiOn(n, fam) { const m = n.pairMulti || {}; return m[fam] !== false; }   /* ברירת מחדל: מאוחד */
function setPairMulti(nid, fam, on) { const n = byId(nid); if (!n) return; n.pairMulti = n.pairMulti || {}; n.pairMulti[fam] = !!on; render(); save(); }
function connectPair(nid) {
  ...
  let made = 0, multis = 0; const N = Math.min(n.panel.holes.length, m.panel.holes.length);
  const len = P.scale ? +(Math.hypot(n.x - m.x, n.y - m.y) * P.scale).toFixed(1) : undefined;
  /* קודם המשפחות המאוחדות: כבל מולטי אחד לכל XLR / רשת / אופטי שטרם חוברו (≥ 2 חורים) */
  const grouped = new Set();
  for (const [fam, d] of Object.entries(PAIR_FAM)) {
    if (!pairMultiOn(n, fam)) continue;
    const idx = []; for (let i = 0; i < N; i++) { const a = n.panel.holes[i]; if (a.conn && a.conn !== 'empty' && pairFamOf(a.conn) === fam && !pairCableOf(nid, i)) idx.push(i); }
    if (idx.length < 2) continue;
    const c = { id: uid('c'), from: nid, to: m.id, type: d.type, qty: '1', spec: '', note: d.n + ' ' + idx.length + '× ' + (fam === 'xlr' ? 'XLR' : fam === 'cat' ? 'Cat6' : 'סיבים'), conn: d.conn, conn2: d.conn2,
      cores: idx.length, chans: idx.map(i => ({ a: i + 1, b: i + 1 })), pOut: 'חורים ' + (idx[0] + 1) + '–' + (idx[idx.length - 1] + 1), pIn: 'חורים ' + (idx[0] + 1) + '–' + (idx[idx.length - 1] + 1), len };
    P.cables.push(c); multis++; idx.forEach(i => grouped.add(i));
  }
  for (let i = 0; i < N; i++) {
    const a = n.panel.holes[i], b = m.panel.holes[i];
    if (grouped.has(i) || !a.conn || a.conn === 'empty' || pairCableOf(nid, i)) continue;
    const ct = pairCabType(n, i), aoc = ct === 'sdi:aoc';
    const c = { id: uid('c'), from: nid, to: m.id, type: aoc ? 'sdi' : ct, qty: '1', spec: aoc ? 'HDMI אופטי (AOC)' : '', note: '', pOut: 'חור ' + (i + 1), pIn: 'חור ' + (i + 1), fromHole: i + 1, toHole: i + 1, conn: a.conn, conn2: b.conn && b.conn !== 'empty' ? b.conn : undefined, len };
    P.cables.push(c); made++;
  }
  render(); save();
  uiToast(made || multis ? '🔗 נוצרו ' + (multis ? multis + ' כבלי מולטי' : '') + (multis && made ? ' + ' : '') + (made ? made + ' כבלים בודדים' : '') + ' בין שני הפאנלים — מופיעים במפתח הכבלים' : 'כל החורים כבר מחוברים');
}
...
function panelEditor(p, nid, ui) {
  ...
  return `
    ...
    ${ui < 0 && byId(nid) && byId(nid).kind === 'panel' ? (() => {
      ...
      const cabOpts = v => Object.entries(CTYPES).filter(([k]) => k !== 'multi').map(([k, t]) => `<option value="${k}" ${k === v ? 'selected' : ''}>${esc(t.n)}</option>`).join('') + `<option value="sdi:aoc" ${v === 'sdi:aoc' ? 'selected' : ''}>HDMI אופטי (AOC)</option>`;
      const famCount = {}; for (let i = 0; i < N; i++) { const f = pairFamOf((n.panel.holes[i] || {}).conn); if (f && !pairCableOf(nid, i)) famCount[f] = (famCount[f] || 0) + 1; }
      const multiBox = Object.entries(famCount).filter(([f, c]) => c >= 2).map(([f, c]) => `<label style="display:flex;gap:6px;align-items:center;font-size:11.5px;margin:2px 0"><input type="checkbox" ${pairMultiOn(n, f) ? 'checked' : ''} onchange="setPairMulti('${nid}','${f}',this.checked)"> 🧵 ${esc(PAIR_FAM[f].n)} אחד עם ${c} ליבות במקום ${c} כבלים בודדים</label>`).join('');
      const LBL = cableLabels();
      const rows = Array.from({ length: N }, (_, i) => { const a = n.panel.holes[i], b = m.panel.holes[i], c = pairCableOf(nid, i); return `<tr>
        ...
        <td>${!c && pairFamOf(a.conn) && pairMultiOn(n, pairFamOf(a.conn)) && famCount[pairFamOf(a.conn)] >= 2 ? `<span style="font-size:11px;color:#1976d2">🧵 ליבה ב${esc(PAIR_FAM[pairFamOf(a.conn)].n)}</span>` : `<select style="width:100%;font-size:11px" onchange="setPairCab('${nid}',${i},this.value)" ${c ? 'disabled' : ''}>${cabOpts(c ? (c.spec && /AOC/.test(c.spec) ? 'sdi:aoc' : c.type) : pairCabType(n, i))}</select>`}</td>
      ...
      return `<h3 class="sec">🔁 הצד השני: ${esc(m.name)}</h3>
        ...
        ${multiBox ? `<div style="margin-top:6px">${multiBox}</div>` : ''}
        <button style="width:100%;margin-top:6px;background:#0f6e56;color:#fff;font-weight:700" onclick="connectPair('${nid}')" ${pending ? '' : 'disabled'}>🔗 חבר בין השניים (${(() => { let k = 0; const fams = {}; for (let i = 0; i < N; i++) { const h2 = n.panel.holes[i]; if (!h2.conn || h2.conn === 'empty' || pairCableOf(nid, i)) continue; const f = pairFamOf(h2.conn); if (f && pairMultiOn(n, f) && famCount[f] >= 2) fams[f] = 1; else k++; } return k + Object.keys(fams).length; })()} כבלים)</button>
...
function stackedBoxes(A, B) {
  ...
  const ovl = Math.min(aR, bR) - Math.max(aL, bL);
  if (ovl < Math.min(A.w, B.w) * 0.35) return null;
  if (B.y >= A.y + (A.h || 0) + 8) return 'AB';   /* A למעלה, B למטה */
  if (A.y >= B.y + (B.h || 0) + 8) return 'BA';
  return null;
}
...
function ioPanelHTML(name, nid, unitId) {
  ...
  if (layout && layout.length) {
    ...
    /* יש תמונת גב — היא הפאנל: לחיצה על מחבר בתמונה מחברת, הכבל מסומן עליו בצבע ובמספר. בלי תמונה — שורות המחברים */
    const im = rearImage(name);
    if (im && (im.url || im.furl)) {
      const mark = it => {
        const isOut = it.port && /^(OUT|LNK)/.test(it.port), isIn = it.port && /^IN/.test(it.port);
        let cc = null;
        if (isOut) cc = P.cables.find(c => c.from === nid && c.fromUnit === unitId && c.pOut === it.port);
      ...
      return rearImageHTML(name, layout, { caption: false, style: 'margin-bottom:4px', mark })
        + (nid ? '<p class="muted" style="font-size:10px;margin-top:3px">💡 לחץ על מחבר בתמונה ואז על מוצר בתכנית — הכבל ישויך למחבר. ✎ גב לעריכה על התמונה.</p>' : '');
    }
...
function patchCss() {
  ...
  st.textContent = `
    ...
    .pchip.src{background:#efecfd;border-color:#c9c0f5;color:#4b3fb8}
    .pchip.src.sel{background:#6c5ce7;color:#fff;border-color:#6c5ce7}
...
function patchCabSel(key) {
  ...
  ensureStock(P);
  let lbl = 'אוטו · ' + (ap ? spkCabLabel(ap).slice(0, 34) : mm + ' ממ״ר');
  if (cur === 'xlr') lbl = 'XLR (סאב מוגבר)';
...
function patchRender() {
  ...
  /* כניסות — עמדות נגינה ומקורות אל הפרוססור/מיקסר שבארון */
  const insHtml = (PATCH.ins || []).map((t, ii) => {
    const rows = [];
    for (let ch = 1; ch <= t.inTotal; ch++) {
      const key = 'i' + ii + '|' + ch, ids = PATCH.inSlots[key] || [];
      rows.push(`<div class="pchCh" data-slot="${key}">
        <span class="pchOut">IN ${ch}</span>
...
function patchInSet(ii, val) {
  ...
  Object.keys(PATCH.inSlots).forEach(kk => {
    const [ii2, ch2] = kk.slice(1).split('|').map(Number);
    if (ii2 === ii && ch2 > v) { PATCH.srcPool.push(...PATCH.inSlots[kk]); delete PATCH.inSlots[kk]; }
  });
  save(); patchRender();
...
async function patchApply() {
  ...
  for (const [key, ids] of Object.entries(PATCH.inSlots || {})) {
    const [ii, ch] = key.slice(1).split('|').map(Number);
    const t = PATCH.ins[ii]; if (!t) continue;
    if ((PATCH.inOrig || {})[key] === ids.join(',')) continue;
  ...
  Object.entries(PATCH.inPre || {}).forEach(([k6, cids]) => {
    ...
    if ((PATCH.inSlots[k6] || []).length) return;
...
/* נתוני רמקול למוקד: לפי השם המלא מההצעה (שם המוקד קטוע ל-40 תווים — "BR121" נהיה "BR12"), ואז לפי שם המוקד, ואז מטריצת ההתאמות */
function spkDataN(n) {
  const full = nodeFullName(n);
  let d = spkData(full) || (full !== n.name ? spkData(n.name) : null);
  if (d) return d;
  if (typeof MATRIX_SPK !== 'undefined') { const nm = spkNorm(shortModel(full)); for (const [k, sp] of Object.entries(MATRIX_SPK.specs || {})) if (nm && spkNorm(k) === nm && (sp.w || sp.o)) return { w: sp.w, o: sp.o, sens: sp.sens, max: sp.spl }; }
  return null;
}
function spkOhm(n) { if (n.band) { const b = spkBandData(nodeFullName(n), n.band); if (b && b.o) return +b.o; } return n.ohm ?? (spkDataN(n)?.o) ?? 8; }
function spkWatt(n) { if (n.band) { const b = spkBandData(nodeFullName(n), n.band); if (b && b.w) return +b.w; } const pw = n.pow ?? (spkDataN(n) || {}).w; return pw == null ? null : +pw; }
...
const AMP_DATA = [
  ...
  { re: /IPX\s?2400/i, kind: 'amp', ch: 2, mo: 2, ok: 1, w: '2×600W @8Ω · 1200W @4Ω · 800W @2Ω · פריסטים ל-K&F · ללא Dante/Lake', pw: { 8: 600, 4: 1200, 2: 800 }, url: 'https://www.kling-freitag.com/proinstall/kf-ipx/kf-ipx/', pdf: 'https://www.kling-freitag.com/content/uploads/ds_ipx_en.pdf' },
  { re: /IPX\s?1200/i, kind: 'amp', ch: 2, mo: 2, ok: 1, w: '2×300W @8Ω · 600W @4Ω · 500W @2Ω · פריסטים ל-K&F', pw: { 8: 300, 4: 600, 2: 500 }, url: 'https://www.kling-freitag.com/proinstall/kf-ipx/kf-ipx/', pdf: 'https://www.kling-freitag.com/content/uploads/ds_ipx_en.pdf' },
  ...
  { re: /IX\s?15:?4\b|\bIX\s?15\b/i, kind: 'amp', ch: 4, mo: 2, ok: 1, w: '4×375W @8Ω/4Ω/2.7Ω/2Ω (כל הערוצים) · ערוץ בודד עד 1000W · גשר 750W · Dante · 70V/100V', pw: { 8: 375, 4: 375, 2.7: 375, 2: 375 }, br: { 8: 750, 4: 750, 2.7: 750, 2: 750 }, dd: { 70: 375, 100: 375 }, url: 'https://www.kling-freitag.com/proinstall/ix-series/ix154/', pdf: 'https://www.kling-freitag.com/content/uploads/ds_ix_en.pdf' },
  ...
  { re: /\bD\s?40:?4\b/i, kind: 'amp', ch: 4, mo: 2, ok: 1, w: '4×1000W @8Ω/4Ω/2.67Ω · 800W @2Ω · 700W @16Ω (כל הערוצים) · ערוץ בודד עד 1750W @4Ω (RPM) · Lake/Dante · 70V/100V', pw: { 8: 1000, 4: 1000, 2.7: 1000, 2: 800 }, dd: { 70: 1000, 100: 1000 }, url: 'https://www.kling-freitag.com/proinstall/d-series/d-40-4/', pdf: 'https://www.kling-freitag.com/content/uploads/ds_d-40-4_en.pdf' },
  { re: /\bD\s?120:?4\b/i, kind: 'amp', ch: 4, mo: 2, ok: 1, w: '4×1900W @8Ω · 3000W @4Ω/2.67Ω/2Ω · 950W @16Ω (כל הערוצים) · ערוץ בודד עד 5900W @2.67Ω (RPM) · Lake/Dante · 70V/100V', pw: { 8: 1900, 4: 3000, 2.7: 3000, 2: 3000 }, dd: { 70: 3000, 100: 3000 }, url: 'https://www.kling-freitag.com/proinstall/d-series/d-120-4/', pdf: 'https://www.kling-freitag.com/content/uploads/ds_d-120-4_en.pdf' },
  /* ===== Funktion-One D-Series (מיוצר ע"י XTA) — טבלת ההספק מדף היצרן funktion-one.com/product-system/d-series ===== */
  { re: /\bD\s?20Q/i, kind: 'amp', ch: 4, mo: 2, ok: 1, w: '4×175W @8Ω · 350W @4Ω · 480W @2.7Ω · 360W @2Ω · גשר 700W @8Ω / 650W @4Ω · DSP 8 ערוצים · Dante אופציונלי', pw: { 8: 175, 4: 350, 2.7: 480, 2: 360 }, br: { 8: 700, 4: 650 }, url: 'https://funktion-one.com/product-system/d-series/' },
  { re: /\bD\s?40Q/i, kind: 'amp', ch: 4, mo: 2, ok: 1, w: '4×500W @8Ω · 1000W @4Ω · 1400W @2.7Ω · 1200W @2Ω · גשר 2000W @8Ω / 2400W @4Ω · DSP 8 ערוצים · Dante אופציונלי', pw: { 8: 500, 4: 1000, 2.7: 1400, 2: 1200 }, br: { 8: 2000, 4: 2400 }, url: 'https://funktion-one.com/product-system/d-series/' },
  { re: /\bD\s?80Q/i, kind: 'amp', ch: 4, mo: 2, ok: 1, w: '4×1000W @8Ω · 2000W @4Ω · 2200W @2.7Ω · 2000W @2Ω · גשר 4000W @8Ω / 4000W @4Ω · DSP 8 ערוצים · Dante אופציונלי', pw: { 8: 1000, 4: 2000, 2.7: 2200, 2: 2000 }, br: { 8: 4000, 4: 4000 }, url: 'https://funktion-one.com/product-system/d-series/' },
  { re: /\bD\s?100Q/i, kind: 'amp', ch: 4, mo: 2, ok: 1, w: '4×1400W @8Ω · 2700W @4Ω · 3700W @2.7Ω · 3500W @2Ω · גשר 5600W @8Ω / 7000W @4Ω · DSP 8 ערוצים · Dante אופציונלי', pw: { 8: 1400, 4: 2700, 2.7: 3700, 2: 3500 }, br: { 8: 5600, 4: 7000 }, url: 'https://funktion-one.com/product-system/d-series/' },
  { re: /\bD\s?120\b(?!\s?:)/i, kind: 'amp', ch: 2, mo: 2, ok: 1, w: '2×2400W @8Ω · 4600W @4Ω · 6250W @2.7Ω · 6800W @2Ω (ללא גשר, ללא DSP)', pw: { 8: 2400, 4: 4600, 2.7: 6250, 2: 6800 }, url: 'https://funktion-one.com/product-system/d-series/' },
  { re: /\bTOPAS\b/i, kind: 'amp', ch: 2, mo: 2, ok: 1, w: '2×500W @8Ω · 1000W @4Ω · 1100W @2Ω · גשר 2000W @8Ω / 2050W @4Ω · מופסק — דף הנתונים הרשמי', pw: { 8: 500, 4: 1000, 2: 1100 }, br: { 8: 2000, 4: 2050 }, url: 'https://www.kling-freitag.com/content/uploads/ds_topas_en.pdf' },
...
/* תמונת מוצר לטבלת הנתונים (רמקולים / מגברים / פרוססורים) — כתובת URL, נשמרת ב-meta של הדגם */
async function dbImgSet(tab, name) {
  const st = tab === 'spk' ? (store.spkMeta = store.spkMeta || {}) : (store.ampMeta = store.ampMeta || {});
  const k = rearKey(name), cur = (st[k] || {}).img || '';
  const v = await uiPrompt('כתובת תמונה (URL) עבור ' + name + '\nריק = חזרה לתמונה האוטומטית מהחנות/היצרן', cur);
  if (v == null) return;
  st[k] = st[k] || {}; if (String(v).trim()) st[k].img = String(v).trim(); else delete st[k].img;
  save(); spkDataManager(tab);
}
...
function renderPanel() {
  ...
  if (n.kind === 'point') {
    ...
    const MOUNTS = ['קיר בלוק', 'קיר בטון', 'תקרה', 'תקרת גבס', 'תקרה מוט הברגה', 'רצפה', 'סטאק', 'בתוך ארון', 'אחר'];
...
function renderLegend() {
  $('#legend').innerHTML = `<h3>KO Projects · מפתח כבלים — ${esc(P.name)}</h3>
    ...
    P.cables.map((c, i) => `<tr>
      ...
      <td>${CTYPES[c.type].n}${c.cores?' · '+c.cores+'× '+coreTxt(c):''}${c.fiber?' · '+c.fiber:''}${c.conn && CONNS[c.conn] ? ' · ' + CONNS[c.conn].n + (c.conn2 && CONNS[c.conn2] && c.conn2 !== c.conn ? ' ← ' + CONNS[c.conn2].n : '') : ''}</td><td>${esc(c.qty)}</td><td>${esc(c.spec)}</td><td>${vdCell(c)}</td><td>${c.inst === 'exist' ? '♻️ קיים' : c.inst === 'pull' ? '🚚 להעברה' : '➕ חדש'}</td><td>${esc(c.note)}</td></tr>`).join('') +
    '</table>' +
...
function routeGroupApply(gk, kind) {
  const sel = window.__rtSel || {}; const cabs = ((window.__rtGroups || {})[gk] || []).filter(c => sel[c.id] !== false); if (!cabs.length) return;
  P.conduits = P.conduits || []; const cd = { id: uid('cd'), kind, name: (kind === 'tray' ? 'תעלה ' : 'צינור ') + (P.conduits.filter(c => c.kind === kind).length + 1), size: kind === 'tray' ? '100×50' : 25, len: Math.max(...cabs.map(c => +c.len || 0)) || '' }; P.conduits.push(cd);
  cabs.forEach(c => { c.route = kind; c.conduit = cd.id; });
  if (kind !== 'tray') { const f = conduitFill(cd); cd.size = f.need || 110; }
  save(); routeManager(); render(); uiToast('🛤 ' + cabs.length + ' קווים → ' + cd.name + (kind !== 'tray' ? ' Ø' + cd.size + ' מ״מ' : ''));
}
window.routeGroupApply = routeGroupApply;
...
function routeManager() {
  ...
  /* קיבוץ אינטואיטיבי: קווים שיוצאים מאותו מקור לאותו מקום (יעדים במרחק ≤ 1.5 מ׳ זה מזה — עמדת DJ, פאנל וסאב ליד) */
  const clusterOf = (() => { const pts = P.nodes.filter(n => !n.hidden).map(n => ({ id: n.id, x: 2200 - n.x - 20, y: n.y + 24 })); const cl = {}; let k = 0; const R = P.scale ? 1.5 / P.scale : 60;
    pts.forEach(p1 => { if (cl[p1.id] != null) return; cl[p1.id] = k; pts.forEach(p2 => { if (cl[p2.id] == null && Math.hypot(p1.x - p2.x, p1.y - p2.y) <= R) cl[p2.id] = k; }); k++; }); return id => cl[id] ?? id; })();
  const gkey = c => { const a = clusterOf(c.from), b = clusterOf(c.to); return [String(a), String(b)].sort().join('|'); };
  /* למטה מקבצים רק קווים שעדיין לא בצינור — מה שכבר שויך מוצג בתוך כרטיס הצינור שלו */
  const free = cabs.filter(c => !P.conduits.some(cd => cd.id === c.conduit));
  ...
  const cdCards = P.conduits.map(cd => { const f = conduitFill(cd), col = conduitColor(cd);
    const sz = cd.kind === 'tray' ? `<select onchange="conduitSet('${cd.id}','size',this.value)">${TRAY_SIZES.map(t => `<option ${cd.size === t ? 'selected' : ''}>${t}</option>`).join('')}</select> מ״מ` : `Ø<select onchange="conduitSet('${cd.id}','size',this.value)">${CONDUIT_DIAS.map(d => `<option value="${d}" ${+cd.size === d ? 'selected' : ''}>${d}</option>`).join('')}</select> מ״מ`;
    return `<div class="rtDrop" data-cd="${cd.id}" ondragover="rtDragOver(event)" ondragleave="this.style.outline=''" ondrop="rtDrop(event,'${cd.id}')" style="border:1.5px solid ${col};border-radius:10px;padding:8px 10px;margin-bottom:8px;background:${f.ok ? '#fff' : '#fdeee8'}" title="גרור לכאן קווים">
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;font-size:12px">
        <span style="display:inline-block;width:14px;height:14px;border-radius:50%;background:${col}"></span><b style="color:${col};font-size:13px">${esc(conduitTag(cd))}</b>
        <input value="${esc(cd.name)}" style="width:110px" onchange="conduitSet('${cd.id}','name',this.value)" title="שם">
        <span>${cd.kind === 'tray' ? '🟫 תעלה' : '🟠 צינור'} ${sz}</span>
        <span>אורך <input value="${esc(cd.len || '')}" placeholder="מ׳" style="width:52px" onchange="conduitSet('${cd.id}','len',this.value)"> מ׳</span>
        <b style="color:${f.ok ? '#0f6e56' : '#c1121f'}">${f.cabs.length} קווים · מילוי ${Math.round(f.pct * 100)}%${f.ok ? ' ✓' : f.need ? ' ⚠ צריך Ø' + f.need : ' ⚠ גדול מדי'}</b>
        <span style="flex:1"></span><button onclick="conduitPickStart('${cd.id}')" title="בחירת קווים בלחיצה על התכנית">🎯 על התכנית</button><button onclick="conduitDel('${cd.id}')" title="מחיקת הצינור (הקווים נשארים, בלי צינור)">✕</button></div>
      <div style="margin-top:5px;min-height:22px">${f.cabs.length ? f.cabs.map(c => chip(c)).join('') : '<span class="muted" style="font-style:italic;font-size:11.5px">ריק — גרור לכאן קווים מלמטה</span>'}</div></div>`; }).join('');
  /* קווים שעדיין בלי צינור — לפי מסלול (מקור→יעד), עם הצעה לצינור אחד */
  const grpRows = Object.entries(groups).sort((a, b) => b[1].length - a[1].length).map(([k, arr]) => {
    const dests = [...new Set(arr.map(c => c.to))].map(nodeNm), srcs = [...new Set(arr.map(c => c.from))].map(nodeNm);
    const sel = window.__rtSel = window.__rtSel || {}; const chosen = arr.filter(c => sel[c.id] !== false); const pr2 = propose(chosen.length ? chosen : arr);
    const chips = arr.map(c => chip(c, `window.__rtSel['${c.id}']=${sel[c.id] === false ? 'true' : 'false'};routeManager()`, sel[c.id] === false)).join('');
    return `<div style="border:1.5px solid #e3ded3;border-radius:10px;padding:8px 10px;margin-bottom:8px;background:#fff">
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><b style="font-size:12.5px;flex:1">${esc(srcs.join(' / ').slice(0, 40))} ← ${esc(dests.join(' · ').slice(0, 60))}${dests.length > 1 ? ` <small style="color:#777">(${dests.length} יעדים סמוכים)</small>` : ''}</b>
        <span style="font-size:11.5px;color:#534ab7">הצעה: צינור ${pr2.need ? 'Ø' + pr2.need + ' מ״מ · מילוי ' + pr2.pct + '%' : 'גדול מ-Ø110 — פצל'}</span>
        <button style="background:#0f6e56;color:#fff;font-weight:700" onclick="routeGroupApply('${k}','conduit')">✓ צינור אחד ל-${chosen.length} הקווים</button><button onclick="routeGroupApply('${k}','tray')">תעלה</button>
        <select onchange="routeGroupOther('${k}',this.value)" title="מעבר בלי צינור/תעלה" style="font-size:11px"><option value="">מעבר אחר…</option>${ROUTE_KINDS.filter(([v]) => v && v !== 'conduit' && v !== 'tray').map(([v, t]) => `<option value="${v}">${t}</option>`).join('')}<option value="__clear">— לא הוגדר —</option></select></div>
      <div style="margin-top:4px">${chips}</div></div>`; }).join('');
  const ov = uiModal(`<div id="routeOv"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><b style="font-size:15px;flex:1">🛤 מסלולי העברה — תעלות וצינורות</b><button data-x>✕</button></div>
    <p class="muted" style="font-size:11.5px;margin:0 0 8px">כמה קווים יכולים לעבור בצינור אחד — לפי מילוי החתך (כבל אחד עד 53%, שניים 31%, שלושה ומעלה 40%; תעלה 50%). צינור מלא מדי מסומן באדום עם הקוטר הדרוש. גוררים קו מכל מקום אל כרטיס הצינור שלו.</p>
    <h3 class="sec" style="display:flex;gap:8px;align-items:center">צינורות ותעלות (${P.conduits.length})<span style="flex:1"></span><button onclick="conduitAdd('conduit')">+ צינור</button><button onclick="conduitAdd('tray')">+ תעלה</button></h3>
    ${cdCards || '<p class="muted" style="font-size:12px">אין עדיין צינורות — אשר הצעה למטה, או הוסף צינור וגרור אליו קווים</p>'}
    <div ondragover="rtDragOver(event)" ondragleave="this.style.outline=''" ondrop="rtDrop(event,'')" style="margin:4px 0 10px;border:1.5px dashed #c9c2b4;border-radius:8px;padding:6px 10px;font-size:11.5px;color:#8a8377;text-align:center">⤵ גרור לכאן קו כדי להוציא אותו מהצינור</div>
    <h3 class="sec" style="display:flex;gap:8px;align-items:center">קווים בלי צינור (${free.length})<span style="flex:1"></span>${Object.keys(groups).length ? `<button style="background:#0f6e56;color:#fff;font-weight:700" onclick="routeApplyAll()">✓ אשר את כל ההצעות</button>` : ''}</h3>
    <p class="muted" style="font-size:11px;margin:0 0 6px">כל כרטיס = קווים מאותו מקור לאותו מקום (יעדים במרחק עד 1.5 מ׳ נחשבים מקום אחד). לחיצה על קו מוציאה אותו מהקבוצה; "✓ צינור אחד" יוצר צינור בקוטר המתאים; אפשר גם לגרור קו ישירות לצינור קיים.</p>
    ${grpRows || '<p class="muted" style="font-size:12px">✓ כל הקווים משויכים</p>'}</div>`);
  ov.querySelector('div').style.maxWidth = '900px'; ov.querySelector('div').style.width = '96vw'; ov.querySelector('div').style.maxHeight = '90vh'; ov.querySelector('div').style.overflowY = 'auto';
  ov.querySelector('[data-x]').onclick = () => ov.remove(); ov.addEventListener('click', e => { if (e.target === ov) ov.remove(); });
}
/* מעבר בלי צינור (גלוי / בקיר / קיים) לקבוצה שלמה */
function routeGroupOther(gk, v) { const sel = window.__rtSel || {}; const arr = ((window.__rtGroups || {})[gk] || []).filter(c => sel[c.id] !== false); arr.forEach(c => { delete c.conduit; if (v && v !== '__clear') c.route = v; else delete c.route; }); save(); routeManager(); renderWires(); }
window.routeGroupOther = routeGroupOther;
function rtDragStart(e, cid) { e.dataTransfer.setData('text/plain', 'cab:' + cid); e.dataTransfer.effectAllowed = 'move'; }
function rtDragOver(e) { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; e.currentTarget.style.outline = '2px dashed #0f6e56'; }
function rtDrop(e, cdId) {
  e.preventDefault(); e.currentTarget.style.outline = '';
  const d = String(e.dataTransfer.getData('text/plain') || ''); if (!d.startsWith('cab:')) return;
  const c = cById(d.slice(4)); if (!c) return;
  if (!cdId) { delete c.conduit; delete c.route; }
  else { const cd = (P.conduits || []).find(x => x.id === cdId); if (!cd) return; c.conduit = cd.id; c.route = cd.kind; const f = conduitFill(cd); if (!f.ok && f.need && cd.kind !== 'tray') uiToast('⚠ ' + cd.name + ' מלא מדי (' + Math.round(f.pct * 100) + '%) — צריך Ø' + f.need); }
  save(); routeManager(); renderWires();
}
window.rtDragStart = rtDragStart; window.rtDragOver = rtDragOver; window.rtDrop = rtDrop;
window.routeManager = routeManager; window.conduitAdd = conduitAdd; window.conduitSet = conduitSet; window.conduitDel = conduitDel; window.cableRouteSet = cableRouteSet;
...
document.addEventListener('pointerdown', e => {
  ...
  if (zd) {
    ...
    if (z) { selZone = z.id; dragZ = { z, sx: e.clientX, sy: e.clientY, ox: z.x, oy: z.y, opoly: z.poly ? z.poly.map(p => ({ ...p })) : null, moved: false, lbl: !!e.target.closest('.zlbl') }; e.preventDefault(); return; }
  }
...
document.addEventListener('pointermove', e => {
  const Z = getZ();
  if (dragZ) {
    if (Math.hypot(e.clientX - dragZ.sx, e.clientY - dragZ.sy) > 3) dragZ.moved = true;
  ...
  if (drag.n.kind === 'point') {
    document.querySelectorAll('.node.droptgt').forEach(el => el.classList.remove('droptgt'));
    const me = document.getElementById('nd_' + drag.n.id); const pe = me ? me.style.pointerEvents : ''; if (me) me.style.pointerEvents = 'none';
    const hit = document.elementFromPoint(e.clientX, e.clientY); if (me) me.style.pointerEvents = pe;
    const nel = hit && hit.closest('.node'); const tn = nel && byId(nel.id.slice(3));
    if (tn && tn.kind === 'rack' && !tn.rear && !tn.min) nel.classList.add('droptgt');
  }
...
document.addEventListener('pointerup', e => {
  ...
  if (drag) {
    ...
    if (movedFar && n.kind === 'point') {
      ...
      const hit = document.elementFromPoint(e.clientX, e.clientY); if (me) me.style.pointerEvents = pe;
      const nel = hit && hit.closest('.node'); const dst = nel && byId(nel.id.slice(3));
      if (dst && dst.kind === 'rack' && !dst.rear && !dst.min) {
        const rails = nel.querySelector('.rails'); let slot = -1;
        if (rails) { const rr = rails.getBoundingClientRect(); slot = Math.max(0, Math.min(dst.ru - 1, Math.floor((e.clientY - rr.top) / (rr.height / dst.ru)))); }
        n.x = moved.ox; n.y = moved.oy;
        pointIntoRack(n.id, dst.id, slot); return;
      }
    }
...
const ZUSE = { 'מוזיקת רקע': '#1D9E75', 'מוזיקה לבר': '#378ADD', 'מסעדה + DJ': '#C9502E', 'הופעות חיות': '#7F77DD', 'מוזיקת ריקודים': '#EF9F27', 'מועדון על מלא': '#E24B4A' };
...
function renderZones() {
  ...
  zs.forEach(z => {
    ...
    if (z.poly) {
      ...
      lbl.title = 'לחיצה — חזרה לתכנון האזור · גרירה — הזזת האזור';
    ...
    d.innerHTML = `<span class="zlbl" data-zdrag="${z.id}" title="לחיצה — חזרה לתכנון האזור · גרירה — הזזת האזור" style="background:${c};cursor:pointer">🗺 ${esc(z.name)}${z.usage ? ' · ' + esc(z.usage) : ''}${dims}</span>
...
function autoZonesHint() {
  /* בלי מפתח API: הבקשה לקלוד (אותם כללים, אותו פורמט פוליגונים) מועתקת ללוח. הכי פשוט — לכתוב ל-Claude Code
  ...
  const ov = uiModal(`<b style="font-size:14px">🤖 זיהוי אזורים בלי מפתח API</b>
    ...
    <ol style="font-size:12.5px;line-height:1.7;padding-inline-start:18px;margin:0 0 8px">
      ...
      <li><b>ידנית דרך claude.ai</b>: הבקשה (הכללים + פורמט הפוליגונים) הועתקה ללוח. הדבק אותה יחד עם צילום התכנית, ואת ה-JSON שחוזר ייבא דרך ייבוא ▾ ← 📦 ייבוא JSON — הוא נכנס כפוליגונים לפי הקירות.</li>
    </ol>
    <p class="muted" style="font-size:11px;margin:0">עם מפתח בשרת (scripts/deploy-gcp.sh envpush אחרי הוספת ANTHROPIC_API_KEY ל-.env) הכפתור עובד לבד, כולל ${(store.zoneExamples || []).length} דוגמאות הלימוד שלך.</p>
    <div style="display:flex;gap:6px;margin-top:10px"><button data-cancel style="flex:1">סגור</button></div>`);
  const c = ov.querySelector('[data-cancel]'); if (c) c.onclick = () => ov.remove();
}
...
/* שיטות ההצבה — מוגדרות פעם אחת ומשמשות גם בפאנל האזור וגם בחלון הקיט */
const PLACE_OPTS = [
  ['ring', '🔄 היקפי — כל הקירות, מרווח קבוע (ברירת מחדל)'],
  ['corners', '4 פינות — פורגראונד/רחבה (Funktion-One)'],
  ['cluster', '🎯 קלאסטר — צביר מרכזי, רמקולים גב אל גב לכל הכיוונים'],
  ['ceiling', 'תקרה — פריסה מבוזרת'],
  ['live', '🎤 במה — מיינים L/R + סאבים + דיליי'],
];
const CLUSTER_N = [2, 3, 4, 6, 8];
function placeOptsHTML(cur) {
  return PLACE_OPTS.map(([v, l]) => `<option value="${v}" ${(cur || 'ring') === v ? 'selected' : ''}>${l}</option>`).join('');
}
function clusterNHTML(cur) {
  return CLUSTER_N.map(n => `<option value="${n}" ${(+cur || 4) === n ? 'selected' : ''}>${n} רמקולים · ${Math.round(360 / n)}° לכל אחד</option>`).join('');
}
...
function zoneSystemBuilder(z) {
  ...
  return `<div style="background:#f4f2ec;border-radius:8px;padding:10px;margin-top:6px">
    ...
    <div class="fld"><label>סוג התקנה</label><select onchange="setZoneField('${zid}','_place',this.value)">
      ${placeOptsHTML(z._place)}
    </select></div>
    ${z._place === 'cluster' ? `<div class="fld"><label>כמה רמקולים בכל קלאסטר</label>
      <select onchange="setZoneField('${zid}','_clusterN',+this.value)">${clusterNHTML(z._clusterN)}</select>
      <p class="muted" style="font-size:10px;margin-top:3px">הרמקולים מוצבים גב אל גב סביב נקודה מרכזית ומפזרים לכל הכיוונים · מספר הקלאסטרים נקבע מהצפיפות</p></div>` : ''}
...
function zoneGridPts(z, spacingPx) {
  const b = zoneBounds(z);
...
function placeZoneSpeakers(z, name, spacingPx, iid, extra) {
  const pts = zoneGridPts(z, spacingPx);
  pts.forEach((p, k) => {
...
function zoneKitConfirm(zname, idx) {
  ...
  const ov = uiModal(`
    ...
    ${z && kitHasSpk ? `<div style="background:#f4f2ec;border-radius:8px;padding:7px 9px;margin-bottom:7px">
      ...
      <div data-clwrap style="margin-top:5px;${z._place === 'cluster' ? '' : 'display:none'}">
        <label style="font-size:11px;font-weight:700;display:block;margin-bottom:3px">כמה רמקולים בכל קלאסטר</label>
  ...
  const plSel = ov.querySelector('[data-place]');
  if (plSel) plSel.onchange = () => {
    setZoneField(z.id, '_place', plSel.value);
    const w = ov.querySelector('[data-clwrap]');
    if (w) w.style.display = plSel.value === 'cluster' ? '' : 'none';
  };
...
function buildZoneSystem(zid) {
  ...
  if (mode === 'cluster') {
    /* קלאסטר — צביר רמקולים גב אל גב בנקודה מרכזית, מפזר 360°.
    ...
    pts.forEach((p, ci) => {
      for (let k = 0; k < N; k++) {
        const ang = k * 360 / N, rad = ang * Math.PI / 180;
        P.nodes.push({
          id: uid('n'), kind: 'point', name: spk + ' (' + (++idx) + ')',
          sub: 'קלאסטר ' + (ci + 1) + ' · ' + z.name,
          x: 2200 - (p.cx + Math.cos(rad) * rPx) - 20, y: (p.cy + Math.sin(rad) * rPx) - 24,
          srcIid: it.iid, mini: true, mount: 'תלייה מרכזית', hgt: Math.max(2.6, ceil - 0.4),
          aim: Math.round(ang), disp, spl,
        });
      }
    });
    it.qty = idx; it.placed = idx; it.zones = { [z.name]: idx }; it.added = true;
    let subMsgC = '';
    if (z._sub) {
      const sit = { on: true, qty: pts.length, name: z._sub, src: 'מערכת אוטו · ' + z.name, key: z._subKey || '', dest: 'point', cat: 'other', u: 1, iid: uid('i'), zones: { [z.name]: pts.length }, placed: pts.length, added: true };
      autoPrice(sit); impItems.push(sit);
...
function zonePlanBack(zid) {
  const zs = P.zones || [];
  if (!zs.length) { uiToast('אין אזורים בתכנית'); return; }
  if (!zid && zs.length > 1) {
    const ov = uiModal(`
      <b style="font-size:14px">🗺 לאיזה אזור לחזור?</b>
      <p class="hint" style="font-size:11.5px;color:#8a8377;margin:6px 0">בחר את האזור שתרצה לתכנן</p>
      ${zs.map(z2 => `<button data-z="${esc(z2.id)}" style="width:100%;margin-bottom:5px;text-align:right;padding:8px 10px;border:1px solid #e2ded4;border-radius:9px;background:#faf8f4;cursor:pointer;font-size:12.5px">🗺 ${esc(z2.name)}${z2.usage ? ' · ' + esc(z2.usage) : ''} · ${zoneAreaM(z2).toFixed(0)} מ"ר${z2._built ? ' · ✓ נבנתה מערכת' : ''}</button>`).join('')}
      <button data-x style="width:100%;margin-top:4px;padding:8px;border-radius:9px;border:1px solid #ddd;background:#fff;cursor:pointer">ביטול</button>`);
    ov.querySelectorAll('[data-z]').forEach(b => b.onclick = () => { ov.remove(); zonePlanBack(b.dataset.z); });
    ov.querySelector('[data-x]').onclick = () => ov.remove();
    ov.addEventListener('click', e => { if (e.target === ov) ov.remove(); });
    return;
  }
...
window.zonePlanBack = zonePlanBack;
...
function exportPDF() {
  ...
  const connDiagram = (cabs, col) => {
    ...
    const CW = 520, GAP = 40 + 44 + 34 * Math.max(3, cabs.length);
    ...
    const row = (arr, below) => { const d = document.createElement('div'); d.style.cssText = 'display:flex;gap:40px;align-items:flex-start;justify-content:center'; arr.forEach(n => { const card = detailCard(n, col, CW, { up: 1.35, titleBelow: below }); if (card) { card.dataset.nid = n.id; card.style.marginBottom = '0'; d.appendChild(card); } }); return d; };
    ...
    for (const c of cabs) {
      ...
      if (c.chans && c.chans.length) {
        /* מולטי: כבל אחד בין הקופסאות, שנפתח בקצה לליבות — כל ליבה לחור שלה */
        const cores = c.chans.map((ch, i) => { const a = pt(A.nid, A.u, fw ? ch.a : ch.b), b = pt(B.nid, B.u, fw ? ch.b : ch.a); return a && b ? { a, b, i } : null; }).filter(Boolean);
        if (cores.length) segs.push({ c, multi: true, cores, a: { nid: A.nid }, b: { nid: B.nid }, lbl: LBLc[c.id], sub: cores.length + ' ליבות' });
        continue;
      }
      const list = c.bands && c.bands.length ? c.bands.map((bd, i) => ({ ha: null, hb: null, pa: bd.port, ua: bd.unitId, lbl: LBLc[c.id] + '.' + (i + 1) })) : [{ ha: fw ? c.fromHole : c.toHole, hb: fw ? c.toHole : c.fromHole, pa: fw ? c.pOut : c.pIn, pb: fw ? c.pIn : c.pOut, lbl: LBLc[c.id] }];
    ...
    const pairs = segs.flatMap(sg => sg.multi ? sg.cores.map(co => ({ a: co.a, b: co.b })) : [{ a: sg.a, b: sg.b }]);
    const fan = (side, other) => { const g = {}; pairs.forEach(pr => { if (!pr[side].el) (g[pr[side].nid] = g[pr[side].nid] || []).push(pr); });
      Object.values(g).forEach(arr => { arr.sort((s1, s2) => s1[other].x - s2[other].x); arr.forEach((pr, i) => { const e = pr[side]; e.x = e.l + (e.r - e.l) * (i + 1) / (arr.length + 1); }); }); };
    fan('a', 'b'); fan('b', 'a');
    /* למולטי: נקודת האיחוד של הליבות (הגזע יוצא ממרכז החורים, קצת מתחת/מעל לשפה) */
    const JD = 22;
    segs.forEach(sg => { if (!sg.multi) return; const mx = arr => arr.reduce((t, v) => t + v, 0) / arr.length;
      sg.a.x = mx(sg.cores.map(co => co.a.x)); sg.a.y = sg.cores[0].a.y + JD; sg.b.x = mx(sg.cores.map(co => co.b.x)); sg.b.y = sg.cores[0].b.y - JD; });
    ...
    const pillW = sg => { const txt = sg.sub ? sg.lbl + ' · ' + sg.sub : sg.lbl; return Math.max(26, txt.length * 6.6 + 12); };
    for (const [cdId, idx] of Object.entries(byCd)) { const cd = (P.conduits || []).find(x => x.id === cdId); if (!cd) continue; const cc = conduitColor(cd); const xs = idx.flatMap(i => { const sg = segs[i], mx = (sg.a.x + sg.b.x) / 2, hw = pillW(sg) / 2; return [sg.a.x, sg.b.x, mx - hw, mx + hw]; }), ys = idx.map(laneY);
      const x0 = Math.min(...xs) - 12, x1 = Math.max(...xs) + 12, y0 = Math.min(...ys) - 11, y1 = Math.max(...ys) + 11; const tg = conduitTag(cd) + ' · ' + idx.length + (idx.length === 1 ? ' כבל' : ' כבלים'); const tw = tg.length * 6.4 + 16;
    ...
    segs.forEach((sg, i) => { const cc = cableColor(sg.c), y = laneY(i); const mx = (sg.a.x + sg.b.x) / 2, txt = sg.sub ? sg.lbl + ' · ' + sg.sub : sg.lbl, pw = pillW(sg);
      if (sg.multi) {
        /* הליבות: קו דק מכל חור אל נקודת האיחוד; הגזע: קו עבה אחד עם המספר */
        sg.cores.forEach(co => { svg += `<path d="M${co.a.x} ${co.a.y} L ${sg.a.x} ${sg.a.y}" fill="none" stroke="${cc}" stroke-width="1.1" opacity="0.75"/><circle cx="${co.a.x}" cy="${co.a.y}" r="2.2" fill="${cc}"/><path d="M${sg.b.x} ${sg.b.y} L ${co.b.x} ${co.b.y}" fill="none" stroke="${cc}" stroke-width="1.1" opacity="0.75"/><circle cx="${co.b.x}" cy="${co.b.y}" r="2.2" fill="${cc}"/>`; });
        svg += `<path d="M${sg.a.x} ${sg.a.y} V ${y} H ${sg.b.x} V ${sg.b.y}" fill="none" stroke="${cc}" stroke-width="5" stroke-linejoin="round" opacity="0.9"/><circle cx="${sg.a.x}" cy="${sg.a.y}" r="4" fill="${cc}"/><circle cx="${sg.b.x}" cy="${sg.b.y}" r="4" fill="${cc}"/>`;
      } else {
        svg += `<path d="M${sg.a.x} ${sg.a.y} V ${y} H ${sg.b.x} V ${sg.b.y}" fill="none" stroke="${cc}" stroke-width="2.2" stroke-linejoin="round"/><circle cx="${sg.a.x}" cy="${sg.a.y}" r="3.2" fill="${cc}"/><circle cx="${sg.b.x}" cy="${sg.b.y}" r="3.2" fill="${cc}"/>`;
      }
      svg += `<rect x="${mx - pw / 2}" y="${y - 8}" width="${pw}" height="16" rx="8" fill="#fff" stroke="${cc}" stroke-width="1.4"/><text x="${mx}" y="${y + 4}" text-anchor="middle" font-size="9.5" font-weight="800" fill="${cc}">${esc(txt)}</text>`; });
    ...
    const out = document.createElement('div'); out.style.cssText = 'text-align:center;margin-top:8px;page-break-inside:avoid'; out.innerHTML = `<div style="font-size:12px;font-weight:700;color:${col};margin-bottom:4px;direction:rtl">🔗 תרשים חיבורים — כל קו מהחור שלו לחור שלו בקופסה השנייה (המספר על הקו = מספר הכבל; מולטי = גזע עבה אחד שנפתח לליבות בקצוות)${Object.keys(byCd).length ? ' · הרצועה המקווקוות = הצינור' : ''}</div>`; out.appendChild(wrap); return out;
  };
  ...
  if (present.length > 1) {
    ...
    for (const cat of ['audio', 'light', 'video', 'data', 'power']) {
      ...
      /* התכנית מימין ולידה (לא מתחתיה) תרשים החיבורים חור-אל-חור — כשיש לפחות שתי קופסאות; אחרת כרטיסי הפירוט */
      ...
      if (diag) { diag.style.cssText += ';flex:1;min-width:260px;margin-top:0'; rowD.appendChild(diag); }
      else if (cards) { right.style.cssText = 'flex:1;min-width:260px;display:flex;gap:12px;flex-wrap:wrap;justify-content:center'; rowD.appendChild(right); }

...
## K&F full-catalog harvest (2026-08-27)
- **Catalog enumeration**: kling-freitag.com is WordPress. Series pages live under `/prorental/<series>/`, `/proinstall/<series>/`, `/proclassics/`. Fetch those three index pages and regex `href="https://www.kling-freitag.com/(prorental|proinstall|proclassics)/[a-z0-9-]+/"` — the model names appear in the page's spec table (model | driver size | coverage). `/proclassics/discontinued-products/` is the legacy list — skip it for current-catalog work.
- **Manual URL slugs** are NOT uniform. Working: `man_gravis-8_en.pdf`, `man_gravis-12_en.pdf`, `man_gravis-15_en.pdf` (one manual covers all coverage variants N/W/XW), `man_scena-12`, `man_sona-6`, `man_sona-8`, `man_nomos-xls`, `man_nomos-xlt`, `man_nomos-lt`, `man_nomos-ls-2`, `man_pia-m`, `man_pia-lfx`, `man_sequenza-5`, `man_sequenza-8`, `man_ca-106-pro`, `man_vida-m`, `man_passio`, `man_spectra-212`. 404: gravis-8w/8xw/12plus-*, sequenza-10, spectra-212-n/xw, sona-8xw, pia-k, vida-c/a. A 404 returns a 109KB HTML page — check the `%PDF` magic bytes, not the byte size.
- **Coverage variants (N/W/XW) share power and impedance** — the GRAVIS 15 manual prints identical 500W/8Ω spec tables for N and W, so a single manual is authoritative for the whole horn family. Note the variant in `notes` rather than skipping it.
- **Newer manuals (2025+) run field and value together** on one extracted line ("Power handling150 W", "Nominal impedance8 Ohm"); older ones (2017) put the value on the NEXT line. Parse both shapes.
- **CA 106**: the `man_ca-106_en.pdf` is a scanned image, but `man_ca-106-pro_en.pdf` has a text layer with the same specs (200W/16Ω/125dB/90°×60°) — always try the PRO/newer variant before declaring a scan-only gap.
- **ERP descriptions can be wrong**: the Hebrew ERP text calls SONA 8 a subwoofer; the manufacturer manual says 2-way top, 85°×55° CD horn. Manufacturer wins.
- **SAE**: sae-audio.com / saeaudio.com refuse connections (no response at all), manuals.plus returns 403 to curl. PCM23II output power remains unsourced — only mains dissipation (3000W) is findable, which is NOT output power.
